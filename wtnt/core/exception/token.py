...
class AuthenticationHeaderSpaceError(AuthenticationFailed):
    default_detail = {"message": "인증 헤더는 반드시 두 덩이로 이루어져야합니다.", "code": "0402"}


class TokenWithNoUserDataError(AuthenticationFailed):
    default_detail = {"message": "토큰에 유저의 식별정보가 들어있지 않습니다.", "code": "0403"}


class UserNotFoundInTokenDataError(AuthenticationFailed):
    default_detail = {"message": "토큰의 유저 식별정보가 유효하지 않습니다.", "code": "0404"}


class UserInactiveInTokenDataError(AuthenticationFailed):
